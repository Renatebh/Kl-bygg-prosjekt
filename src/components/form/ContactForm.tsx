...
const ContactForm = () => {
  ...
  const schema: ZodType<FormData> = z.object({
    name: z.string().min(2, { message: "Name must be at least 2 characters long" }).max(30).trim(),
    email: z.string().email({ message: "Please enter a valid email" }).min(5).trim(),
  ...
  return (
    <form action='post' onSubmit={handleSubmit(submitData)} className={styles['form']}>
